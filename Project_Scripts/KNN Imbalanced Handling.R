...
confusionMatrix(predict_tog, dtrain_tog$Drafted)
...
confusionMatrix(predict_tog_under, dtrain_tog$Drafted)
...
confusionMatrix(predict_tog_over, dtrain_tog$Drafted)
...
confusionMatrix(predict_tog_rose, dtrain_tog$Drafted)
...
confusionMatrix(predict_tog_smote, dtrain_tog$Drafted)
...
for (name in names(models)) {
  ...
  comparison[comparison$model == name, ] <- filter(comparison, model == name) %>%
    mutate(Sensitivity = model$byClass["Sensitivity"],
           ...
           Accuracy = model$byClass["Precision"],
